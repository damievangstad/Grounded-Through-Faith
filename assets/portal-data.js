(function () {
  ...
  function normalizeCustomCollection(list) {
    ...
    return list
      ...
      .map((entry) => ({
        title: 'Faith Formation AI Study',
        steps: Array.isArray(entry.steps) && entry.steps.length > 0
          ? entry.steps
  ...
  function upsertCustomStudy(state, study) {
    ...
    const normalizedStudy = {
      title: 'Faith Formation AI Study',
      steps: Array.isArray(study.steps) && study.steps.length > 0
        ? study.steps
        : [{ id: `${study.id}-step-1`, title: 'First step' }],
      stepsProgress: {},
      link: study.link || (study.id ? `custom-study.html?id=${encodeURIComponent(study.id)}` : undefined),
      ...study,
    };
    ...
    if (existingIndex >= 0) {
      state.studies.custom[existingIndex] = {
        ...
        ...normalizedStudy,
        stepsProgress: {
          ...(state.studies.custom[existingIndex].stepsProgress || {}),
  ...
  function removeCustomStudy(state, id) {
